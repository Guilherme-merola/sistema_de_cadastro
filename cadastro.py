...
class Cadastro(tk.Frame):
    ...
    def design(self: object) -> None:
        ...
        self.label_email.grid(row=2, column=0, padx=15, pady=5)
        self.input_email = tk.Entry(self)
        self.input_email.grid(row=2, column=1, padx=15, pady=5)

        #Mensagem
        self.label_mensagem = tk.Label(self, text="")
        self.label_mensagem.grid(row=3, column=1, padx=15, pady=10)

        #Botão
        self.botao_cadastrar = tk.Button(self, text="Cadastrar", command=lambda: self.cadastrar(self.input_login.get(),
                                                                                           self.input_senha.get(),
                                                                                           self.input_email.get()))
        self.botao_cadastrar.grid(row=3, column=0, padx=15, pady=10)
        self.botao_voltar = tk.Button(self, text="Voltar", command=lambda: self.master.trocar_tela("login"))
        self.botao_voltar.grid(row=3, column=1, padx=15, pady=10)

    def cadastrar(self: object, login: str, senha: str, email: str) -> tk:
        """
        Método responsável por chamar o master e executar seu método de
        cadastrar pessoa.
        """
        return self.master.cadastrar(login, senha, email)
    
    def exibir_mensagem(self: object, texto: str) -> tk:
        """
        Método resposável por exibir uma mensagem de erro no label_mensagem.
        """
        self.botao_cadastrar.grid(row=4, column=0, padx=15, pady=10)
        self.botao_voltar.grid(row=4, column=1, padx=15, pady=10)
